...
1-minute timeframe market data and trade executions, including buyer/seller volumes.
...
class DataProcessor:
    ...
    def __init__(self):
        ...
        self.second_trade_volumes: Dict[int, Dict[str, float]] = defaultdict(lambda: {'buyer_volume': 0.0, 'seller_volume': 0.0})
        self.current_minute_start: Optional[int] = None

    def add_trade(self, trade: TradeData):
        """Add trade to buffer and update second-level counts and volumes"""
        ...
        if trade.is_buyer_maker:
            ...
            self.second_trade_volumes[second_timestamp]['seller_volume'] += trade.quantity
        else:
            self.second_trade_counts[second_timestamp]['buyers'] += 1
            self.second_trade_volumes[second_timestamp]['buyer_volume'] += trade.quantity
    ...
    def process_minute_candle(self, kline_data: Dict) -> Optional[CandleData]:
        ...
        # Calculate buyer and seller volume for the candle duration
        buyer_volume = sum(trade.quantity for trade in minute_trades if not trade.is_buyer_maker)
        seller_volume = sum(trade.quantity for trade in minute_trades if trade.is_buyer_maker)

        # Power position calculation
        ...
        cutoff_time = close_time - (2 * 60 * 1000)
        ...
        for second in list(self.second_trade_counts.keys()):
            if second < cutoff_second:
                ...
                del self.second_trade_volumes[second]
    ...
    def _init_csv(self):
        ...
        with open(self.csv_path, 'w', newline='') as file:
            ...
            writer.writerow([
                ...
                'buyer_volume', 'seller_volume'  # New fields
            ])
    ...
    def _save_to_csv(self, candle: CandleData):
        ...
        with open(self.csv_path, 'a', newline='') as file:
            ...
            writer.writerow([
                ...
                candle.num_buyers, candle.num_sellers, candle.power_position,
                candle.max_buyers_per_second, candle.max_sellers_per_second,
                candle.buyer_volume, candle.seller_volume
            ])
    ...
    def _save_to_sqlite(self, candle: CandleData):
        ...
        cursor.execute('''
            ...
            (timestamp, open_price, high_price, low_price, close_price, volume,
             num_buyers, num_sellers, power_position, max_buyers_per_second, max_sellers_per_second,
        ...
        ''', (
            ...
            candle.power_position, candle.max_buyers_per_second, candle.max_sellers_per_second,
            candle.buyer_volume, candle.seller_volume
        ))
    ...
    async def _handle_message(self, message: str):
        ...
        try:
            ...
            if not isinstance(data, dict):
                self.logger.warning(f"Received non-dict JSON data: {data}")
                return

            if 'stream' not in data or 'data' not in data:
                self.logger.warning(f"Message missing expected keys: {data}")
                return
                ...
                if kline_data['x']:  # Candle is closed
                    ...
                    if candle:
                        ...
                        self.logger.info(
                            ...
                            f"Buyers:{candle.num_buyers} Sellers:{candle.num_sellers} Power:{candle.power_position} "
                            f"BuyerVol:{candle.buyer_volume:.4f} SellerVol:{candle.seller_volume:.4f}"
                        )
            else:
                self.logger.debug(f"Ignored unknown stream message: {stream_name}")

        except json.JSONDecodeError:
            self.logger.warning(f"Received non-JSON message: {message}")
        except Exception as e:
            self.logger.error(f"Error handling message: {e} - Raw message: {message}")
